...
if (oauthForm) {
    ...
    async function loadConfig() {
        try {
            ...
            // Load OAuth Integration Status
            try {
                const statusRes = await apiCall('/api/v1/oauth/status');
                if (statusRes && statusRes.data) {
                    const st = statusRes.data;
                    const setupBadge = document.getElementById('github-setup-badge');
                    const connBadge = document.getElementById('github-connection-badge');
                    ...
                    const reconnectBtn = document.getElementById('github-reconnect-btn');
                    const scopeInfo = document.getElementById('github-scope-info');
                    const scopeList = document.getElementById('github-scope-list');

                    const syncBtn = document.getElementById('github-sync-repos-btn');

                    if (st.is_configured) {
                        if (setupBadge) {
                            setupBadge.className = 'status-badge synced';
                            setupBadge.textContent = 'App Configured';
                        }
                    } else {
                        if (setupBadge) {
                            setupBadge.className = 'status-badge pending';
                            setupBadge.textContent = 'App Not Configured';
                        }
                    }

                    if (st.is_user_connected) {
                        if (connBadge) {
                            connBadge.className = 'status-badge success';
                            connBadge.textContent = 'Connected as @' + (st.connected_username || 'user');
                        }
                        if (desc) {
                            desc.textContent = 'Account linked to GitHub (' + (st.connected_email || '') + '). Private & public repositories auto-sync on login.';
                        }
                        if (icon) {
                        ...
                        if (connBadge) {
                            connBadge.className = 'status-badge cancelled';
                            connBadge.textContent = 'Not Logged In with GitHub';
                        }
                        if (desc) {
                            desc.textContent = st.is_configured 
                                ? 'GitHub App is setup. Log in or connect your GitHub account to sync private and public repositories.' 
                                : 'GitHub Integration is not yet configured. Use automatic setup or enter Client ID & Secret below.';
                        }
                        if (icon) {
                            icon.innerHTML = '<i data-lucide="alert-circle" class="text-warning"></i>';
                        }
                        if (connectBtn) connectBtn.style.display = st.is_configured ? 'inline-flex' : 'none';
                        if (reconnectBtn) reconnectBtn.style.display = 'none';
                        if (syncBtn) syncBtn.style.display = 'none';
                    }

                    if (st.scopes && st.scopes.length > 0) {
                        if (scopeInfo) scopeInfo.style.display = 'block';
                        if (scopeList) scopeList.textContent = st.scopes.join(', ');
                    }
    ...
    window.syncGitHubReposNow = function() {
        showToast('Syncing repositories from GitHub...', 'info');
        window.location.href = '/api/v1/oauth/github/login?setup=1';
    };
